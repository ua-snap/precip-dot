# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
# Fudge final confidence interval values to ensure that all interval bounds
# at a single point consistently increases as duration and return interval
# increase.
#
...
def iterate_durations():
    ...
    for (i, higher_dur) in enumerate(DURATIONS[1:], start=1):
        ...
        higher_files = glob(
            os.path.join(src_dir, f"*_{data_group}_sum_wrf_{higher_dur}_*.nc")
        )
        ...
        for higher_file in higher_files:
            ...
            higher_file_out = os.path.join(
                out_path,
                os.path.basename(higher_file).replace("_undiff.nc", "_fudgeci.nc"),
            )
            ...
            if i == 1:
                lower_file_out = os.path.join(
                    out_path,
                    os.path.basename(lower_file).replace("_undiff.nc", "_fudgeci.nc"),
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Compute deltas for historical vs. projected data."
    )
    parser.add_argument(
        "-p",
        "--path",
        ...
        dest="path",
        type=str,
        help="input directory storing the return interval data.",
    )
    parser.add_argument(
        "-o",
        "--out_path",
        ...
        dest="out_path",
        type=str,
        help="output directory to write outputs",
    )
    parser.add_argument(
        "-d",
        "--data_group",
        ...
        dest="data_group",
        type=str,
        help="name of the model to use: either 'NCAR-CCSM4' or 'GFDL-CM3'",
    )
    parser.add_argument(
        "-n",
        "--ncpus",
