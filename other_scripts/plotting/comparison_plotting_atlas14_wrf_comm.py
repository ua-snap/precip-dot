...
for community in ['Seward', 'Barrow', 'Nome','Anchorage', 'Fairbanks', 'Juneau', 'Dillingham']: 
	...
	atlas_mean.to_csv(os.path.join(base_path,'compare_atlas14_wrf','pr_freq_noaa-atlas14_ak_annualmaximum_area_wide_mean_{}.csv'.format(community)))
	...
	diff.to_csv(os.path.join(base_path,'compare_atlas14_wrf','pf_compare_area_wide_mean_{}.csv'.format(community)))
	diff.plot(kind='line', title='Comparison WRF - Atlas14 Differences Area-wide Mean\nPrecip Frequency -- {}'.format(community),figsize=(16,9))
	plt.savefig(os.path.join(base_path,'compare_atlas14_wrf','pf_compare_area_wide_mean_{}.png'.format(community)))
	plt.close()
	plt.cla()
	...
	atlas.values[ind] = np.nan # update the out-of-bounds values in the atlas data
	atlas_mean = pd.DataFrame({i:atlas.sel(intervals=i).isel(xc=rowcols[community][0],yc=rowcols[community][1]).values.copy() for i in intervals}, index=durations)
	atlas_mean.to_csv(os.path.join(base_path,'compare_atlas14_wrf','pr_freq_noaa-atlas14_ak_annualmaximum-lowerlimit_area_wide_mean_{}.csv'.format(community)))
	...
	wrf = wrf.sel(durations=durations) # make them common
	wrf_mean = pd.DataFrame({i:wrf.sel(intervals=i).isel(xc=rowcols[community][0],yc=rowcols[community][1]).values.copy() for i in intervals}, index=durations)
	wrf_mean.to_csv(os.path.join(base_path,'compare_atlas14_wrf','pf_lower-ci_ERA-Interim_historical_area_wide_mean_{}.csv'.format(community)))
	...
	diff = wrf_mean - atlas_mean
	# diff[atlas == -9999] = np.nan
	# mean = {i:diff.sel(intervals=i).values for i in intervals} # .mean(axis=(1,2))
	# mean_df = pd.DataFrame(mean, index=durations)
	diff.to_csv(os.path.join(base_path,'compare_atlas14_wrf','pf-lowerlimit_compare_area_wide_mean_{}.csv'.format(community)))
	diff.plot(kind='line', title='Comparison WRF - Atlas14 Differences Area-wide Mean\nLower Confidence Bound -- {}'.format(community), figsize=(16,9))
	plt.savefig(os.path.join(base_path,'compare_atlas14_wrf','pf-lowerlimit_compare_area_wide_mean_{}.png'.format(community)))
	plt.close()
	plt.cla()
	...
	atlas.values[ind] = np.nan # update the out-of-bounds values in the atlas data
	atlas_mean = pd.DataFrame({i:atlas.sel(intervals=i).isel(xc=rowcols[community][0],yc=rowcols[community][1]).values.copy() for i in intervals}, index=durations)
	atlas_mean.to_csv(os.path.join(base_path,'compare_atlas14_wrf','pr_freq_noaa-atlas14_ak_annualmaximum-upperlimit_area_wide_mean_{}.csv'.format(community)))
	...
	wrf = wrf.sel(durations=durations) # make them common
	wrf_mean = pd.DataFrame({i:wrf.sel(intervals=i).isel(xc=rowcols[community][0],yc=rowcols[community][1]).values.copy() for i in intervals}, index=durations)
	wrf_mean.to_csv(os.path.join(base_path,'compare_atlas14_wrf','pf_upper-ci_ERA-Interim_historical_area_wide_mean_{}.csv'.format(community)))
	...
	# mean = {i:diff.sel(intervals=i).values for i in intervals}
	# mean_df = pd.DataFrame(mean, index=durations)
	diff.to_csv(os.path.join(base_path,'compare_atlas14_wrf','pf-upperlimit_compare_area_wide_mean_{}.csv'.format(community)))
	diff.plot(kind='line', title='Comparison WRF - Atlas14 Differences Area-wide Mean\nUpper Confidence Bound -- {}'.format(community), figsize=(16,9))
	plt.savefig(os.path.join(base_path,'compare_atlas14_wrf','pf-upperlimit_compare_area_wide_mean_{}.png'.format(community)))
	plt.close()
	plt.cla()
