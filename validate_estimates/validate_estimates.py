...
def read_a14(fp):
    ...
    pf_upper = pd.read_csv(
        fp, skiprows=40, nrows=15, header=None, index_col=0, usecols=cols
    )
    pf_lower = pd.read_csv(
        fp, skiprows=62, nrows=15, header=None, index_col=0, usecols=cols
    )
...
def plot_deltas(wrf_dir, val_dir, wrf_mod):
    ...
    # lookup table for location names
    names_di = {
        "PANC": "Anchorage",
        "PFYU": "Fort Yukon",
        "PASC": "Deadhorse",
    ...
    for duration in durations:
        ...
        plt_df = pd.DataFrame()
        wrf = xr.open_dataset(
            os.path.join(wrf_dir, wrf_fn.format(wrf_mod, duration))
        ).load()
        for stid in stids:
            sname = names_di[stid]
            # get stid, coordinates
            # stid = a14_fn[:4]
            coords = locations[["lon", "lat"]][
                locations["stid"] == stid
            ].values.flatten()
            ...
            # data = {'name': names_di[stid], 'ri': ri, 'estimate_type': estimate_type, 'delta': deltas}
            data = {
                "sname": sname,
                "ri": ri,
...
def write_html(val_dir):
    ...
    base_fn = "deltas_{}_{}.png"
    # collect tags for all plot images
    img_tags = []
    for mod in wrf_mods:
        for dur in durations:
            fn = base_fn.format(mod, dur)
    ...
    NOAA Atlas 14 webtool for 10 Alaskan locations to check the historical WRF-based PF \
    estimates that have been produced. The following plots depict the 'deltas' of estimates \
    between these two sources, computed as the Atlas 14 estimate divided by the WRF-based estimate. This is done for \
    all return intervals between 2y and 1000y (x-axis) and durations between 60m and 60d, for \
    all three models. 
    </p>
